...
const ChatContainer = () => {
  ...
  const {messages, selectedUser, setSelectedUser, sendMessage, getMessages, setUnseenMessages} = useContext(ChatContext)
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg,index)=>(
          <div key ={index} className={`flex items-end gap-2 justify-end ${msg.sender!== authUser._id && 'flex-row-reverse'}`}>
            {msg.image?(
              <img src ={msg.image} alt=" " className = 'max-w-[230px] border borde-gray-700 rounded-lg overflow-hidden mb-8'/>
            ):(
